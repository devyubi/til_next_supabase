...
- /auth/callback?next=`비밀번호 수정 될 라우터 경로`
- env : `NEXT_PUBLIC_APP_URL` 추가
...
- route.ts 는 오로지 서버 컴포넌트에서만 작성 가능함
- Supabase 는 웹브라우저에서 처리됨
- Next.js 는 서버라서, 웹브라우저에 접근이 불가함.
- 즉, 웹 브라우저에서 보관해줘야 할 Supabase 의 쿠키를 생성 할 수 없음.
- 그래서 Next.js 에서 조금 복잡한 과정으로 Supabase 인증 키를 구워주어야 함.
- 그래서 route.ts 를 활용함.

```ts
/**
 * 아래 주의 사항 : 서버에서 클라이언트로 접근해야함
...
- /src/lib/supabase/server.ts : 서버 컴포넌트에서 활용 (서버용, Next.js 등 다른곳도 연동 가능) / 비동기 (await 활용)
- /src/lib/supabase/middleware.ts : Next.js 와 연동하는 경우 활용 / 동기

## 6. supabase 의 middleware.ts

- 화면에 페이지 즉, app/layout.tsx 를 그려내기 전에 사전에 처리가 필요함
- `/src/lib/supabase/middleware.ts` 리턴값 업데이트
- Cookie 관리, Next.js 의 middleware 에 활용
...
- 반드시, `middleware.ts` 라는 파일명은 약속 되어있음
- 예) layout.tsx, page.tsx, loading.tsx 등등..

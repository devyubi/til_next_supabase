...
- `/src/components/post/PostItem.tsx 파일` 분리
- 수정 버튼 부분을 별도로 컴포넌트로 분리함
...
- `/src/components/post/PostItem.tsx 파일` 변경
...
### 1.1. 기본 Editor Modal 의 문제점

- `ADD`, `EDIT` 구분 필요
- `/src/components/post/EditPostItemButton.tsx` 업데이트
...
- 필요로한 Editor Modal 에 전달할 데이터 모양을 고려함

### 1.3. 실제 데이터 Props 로 받아서 전달해 보자

- `/src/components/post/PostItem.tsx` 업데이트 (Props 전달)
...
- `/src/components/post/EditPostItemButton.tsx` 업데이트 (Props 적용)
...
- `/src/apis/post.ts` 추가
- 기존의 API 재활용 가능
- 새글등록 → 포스트등록 → 파일등록 → `Post 업데이트` 기존에 구현해놓음
...
- `/src/hooks/muations/post/useUpdatePost.ts 파일` 생성

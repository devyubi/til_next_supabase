...
- 단계 : 포스트 등록 → 포스트의 ID를 알아냄 → 파일 업로드 → 파일의 URL → 포스트 업데이트
- 생성되는 구조 : `사용자 ID 폴더 / 포스트 ID / 파일들 ···`
...
- `/src/hooks/mutations/post/useCreatePost.ts` 함수 변경
...
- 테스트 해보기 : posts 테이블, uploads

## 4. 저장 내용 임시 저장하기 (작성 중 닫힘 방지)
...
- `/src/components/modal/AlertModal.tsx` 업데이트
...
- `/src/components/providers/ModalProvider.tsx` 업데이트
...
- `/src/components/post/PostFeed.tsx` 에 배치해서 `디자인 보기`

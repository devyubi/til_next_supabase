...
- 반드시 `@import 는 css 첫 줄` 이어야 함
- tailwind 4.x 버전이므로 `@import "tailwindcss";` 들어감
...
- `Prettier - Code formatter 설치` 확장 프로그램 필요
...
- 포멧팅 규칙 정의 설정 (json 파일은 아니지만 임시로 json이라고 해놈..)
...
- 반드시 있어야 함.
- 포멧팅에서 제외할 파일들을 명시함 (이게 없으면 node_modules 도 정리하려고 들음)
...
### 4.5. 명령어로 포멧팅을 한번에 실행하도록 스크립트 작성 (선택사항)
...
rules: {
    ...
    // "prefer-const": "warn",                       // const 사용을 경고로 변경
  }
```

- 자세한 옵션을 포함한 예제 (추후 필요한 부분 추출하여 사용)
...
- 현재는 이렇게만 세팅. (전체 eslint.config.mjs)
...
- ESLint 와 Prettier 가 충돌하지 않도록 설정
...
- `/.vscode/extensions.json` 확장 프로그램 정의 내용 작성 (파일을 만들진 않음. 설명만!)
...
- Editor 에 일관성을 유지하는 내용 작성 파일 (파일을 만들진 않음. 설명만!)
...
git remote add origin https://github.com/devyubi/til_next_supabase.git
```
...
```bash
git config --global user.name "devyubi"
git config --global user.email "dev.munyubi@gmail.com"
```
